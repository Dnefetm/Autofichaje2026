...
// ── Utilidad: similitud entre dos strings (Dice coefficient) ─────────────
...
export default function MappingModal({ listing, onClose, onSuccess }: MappingModalProps) {
    ...
    // SKU de la publicacion para matching
    ...
    // Carga sugerencias inteligentes: busca articulos cuyo SKU/codigo sea similar al de la publicacion
    async function loadSmartSuggestions() {
        const ref = pubSku || pubEan || pubGtin || pubUpc || pubModel;
        if (!ref) return;
        setSuggestionsLoading(true);
        try {
            // Buscar por coincidencia exacta primero, luego parcial
            const { data, error } = await supabase
                .from('articulos')
                .select('articulo_id, nombre, marca, modelo, variante, codigo_universal, sku, caja_madre')
                .not('nombre', 'like', '%PLACEHOLDER%')
                .or(`articulo_id.ilike.%${ref}%,sku.ilike.%${ref}%,codigo_universal.ilike.%${ref}%,nombre.ilike.%${ref}%`)
                .limit(20);
            if (error) throw error;
            if (data && data.length > 0) {
                // Ordenar por similitud al SKU de la publicacion
                const scored = data.map(item => {
                    const skuSim = Math.max(
                        stringSimilarity(ref, item.articulo_id || ''),
                        stringSimilarity(ref, item.sku || ''),
                        stringSimilarity(ref, item.codigo_universal || ''),
                        stringSimilarity(pubEan, item.codigo_universal || ''),
                        stringSimilarity(pubGtin, item.codigo_universal || '')
                    );
                    // Bonus por coincidencia de marca
                    const brandBonus = pubBrand && item.marca && pubBrand.toLowerCase() === item.marca.toLowerCase() ? 0.15 : 0;
                    return { ...item, _score: skuSim + brandBonus };
                });
    ...
    async function searchPhysicalCatalog() {
        try {
            ...
            // Ordenar resultados por similitud al SKU de la pub
    ...
    // Filtrar sugerencias que ya estan seleccionadas
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
                    ...
                    {/* Tarjeta de Publicacion Origen - MEJORADA */}
                    <div className="bg-gradient-to-r from-slate-50 to-indigo-50 rounded-xl p-4 border border-slate-200">
                        <div className="flex gap-4">
                            {listing.url_imagen && (
                                <img src={listing.url_imagen} alt="Producto" className="w-20 h-20 object-contain rounded-lg bg-white border border-slate-200 flex-shrink-0" />
                            )}
                            <div className="flex-1 min-w-0">
                                ...
                                {/* Identificadores detallados */}
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                    {pubSku && (
                                        <span className="inline-flex items-center gap-1 text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-md font-mono">
                                            <Tag size={10} /> SKU: {pubSku}
                                        </span>
                                    )}
                                    {pubEan && (
                                        <span className="inline-flex items-center gap-1 text-[10px] bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-md font-mono">
                                            <Barcode size={10} /> EAN: {pubEan}
                                        </span>
                                    )}
                                    {pubGtin && pubGtin !== pubEan && (
                                        <span className="inline-flex items-center gap-1 text-[10px] bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-md font-mono">
                                            <Barcode size={10} /> GTIN: {pubGtin}
                                        </span>
                                    )}
                    ...
                    {(suggestionsLoading || filteredSuggestions.length > 0) && (
                        <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                            <h4 className="text-xs font-bold text-green-800 flex items-center gap-1.5 mb-2">
                                ...
                                {suggestionsLoading ? 'Buscando coincidencias...' : `${filteredSuggestions.length} sugerencia${filteredSuggestions.length !== 1 ? 's' : ''} por similitud de SKU`}
                            </h4>
                            {!suggestionsLoading && filteredSuggestions.slice(0, 5).map(res => (
                                <button key={res.articulo_id} onClick={() => handleAddSku(res)}
                                    className="w-full text-left p-2 mb-1 rounded-lg hover:bg-green-100 border border-transparent hover:border-green-300 transition-all flex items-center justify-between group">
                                    <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {res._score >= 0.8 && <span className="text-[9px] bg-green-200 text-green-800 px-1.5 py-0.5 rounded-full font-bold">Match alto</span>}
                                        {res._score >= 0.5 && res._score < 0.8 && <span className="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">Parcial</span>}
                            ...
                            selectedSkus.map(s => (
                                <div key={s.sku} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-lg mb-2 hover:shadow-sm transition-shadow">
                                    ...
                                    <div className="text-center">
                                        <span className="text-[10px] text-slate-400 block mb-1">Cantidad por unidad vendida</span>
