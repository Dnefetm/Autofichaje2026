...
export default function ErrorBoundary({ error, reset }: { error: Error & { digest?: string }, reset: () => void }) {
    ...
    return (
        <div className="flex-1 min-h-screen p-6 flex flex-col items-center justify-center bg-[var(--bg)] text-[var(--text)]">
            <div className="bg-[var(--surface)] p-8 rounded-2xl border border-[var(--err)]/30 shadow-sm max-w-md w-full text-center">
                <AlertCircle className="w-12 h-12 text-[var(--err)] mx-auto mb-4" />
                ...
                <div className="bg-[var(--surface-2)] rounded p-3 text-left mb-6 overflow-auto max-h-32">
                    <p className="text-xs font-mono text-[var(--err)] break-all">{error.message}</p>
                </div>
                <button 
                    onClick={() => reset()} 
