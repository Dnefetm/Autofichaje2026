...
const co = conteos[origen];
const cd = conteos[destino];
let estado: string;
if (co === 0) estado = 'sin_uso';
else if (cd === 0) estado = 'rota';
else estado = 'viva';
...
return NextResponse.json({ ok: true, generado: new Date().toISOString(), conteos, aristas, huerfanas, resumen: { tablas: TABLAS.length, aristas: aristas.length, rotas: aristas.filter(a => a.estado === 'rota').length, sin_uso: aristas.filter(a => a.estado === 'sin_uso').length } });
} catch (e: unknown) {
