...
// Patrones de URLs que NO son imágenes de producto (logos, iconos, UI, pagos, redes)
const SKIP_PATTERNS = [
...
// Decodifica URLs envueltas por /_next/image?url=
...
// TIER 1: imágenes declaradas como producto (JSON-LD + og:image). Las más fiables.
...
// TIER 2: barrido genérico de <img>/srcset (fallback con más ruido).
...
// Filtramos ambos tiers contra los patrones de basura.
const primaryClean = [...primary].filter(isLikelyProductImage);
const secondaryClean = [...secondary].filter(isLikelyProductImage);

// Si hay imágenes primarias, esas mandan; si no, usamos el fallback.
let candidatas = primaryClean.length > 0
? [...new Set([...primaryClean, ...secondaryClean])]
: secondaryClean;

// Validamos con HEAD que sean imágenes reales accesibles.
const validImages: string[] = [];
for (const imgUrl of candidatas.slice(0, 50)) {
try {
...
ok: true,
imagenes: [],
advertencia: 'No se encontraron imágenes de producto en la página. Verifica la URL.',
});
}
...
confianza: index === 0 ? 99 : 80,
}));

return NextResponse.json({ ok: true, imagenes: resultado });
}
