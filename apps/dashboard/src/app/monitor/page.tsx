...
export default function MonitorPage() {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] shadow-sm overflow-hidden text-sm">
                <div className="p-4 border-b border-[var(--border)] flex justify-between items-center bg-[var(--bg)]">
                    ...
                    <button onClick={fetchJobs} className="text-[var(--accent)] font-medium hover:underline flex items-center gap-1">
                ...
                <div className="divide-y divide-[var(--border)]">
                    {loading && !jobs.length ? (
                        <div className="p-8 text-center text-[var(--text-faint)]">Cargando monitor...</div>
                    ) : jobs.length === 0 ? (
                        <div className="p-8 text-center text-[var(--text-faint)]">No hay actividad reciente en la cola de jobs.</div>
                    ) : jobs.map((job) => (
                        <div key={job.id} className="p-4 flex items-center justify-between hover:bg-[var(--bg)] transition-colors">
                            <div className="flex items-center gap-4">
                                <StatusIcon status={job.status} />
                                <div>
                                    <div className="font-bold flex items-center gap-2">
                                        {job.type}
                                        <span className="text-[10px] font-mono font-normal bg-[var(--surface-2)] text-[var(--text-muted)] px-1.5 py-0.5 rounded">
                                            ID: {job.id.slice(0, 8)}
                                        </span>
                                    </div>
