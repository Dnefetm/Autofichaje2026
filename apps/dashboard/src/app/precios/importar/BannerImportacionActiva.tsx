  ...
  return (
    <div className="bg-[var(--warn)]/10 border border-[var(--warn)]/30 rounded-xl p-4 sm:p-5 flex flex-col gap-4 shadow-sm relative overflow-hidden">
      <div className="flex gap-3">
        <div className="text-[var(--warn)] mt-0.5 shrink-0">
          <AlertCircle className="w-5 h-5" />
        </div>
        <div>
          <h3 className="text-[var(--warn)] font-bold text-sm">Hay una importación en curso para este proveedor</h3>
          <p className="text-[var(--warn)] text-sm mt-1">
            ...
            <br />
            Estado de tabla: <span className="font-semibold uppercase tracking-wide text-xs bg-[var(--warn)]/20 px-1 py-0.5 rounded mr-2">{activa.estado}</span>
      ...
      <div className="flex flex-wrap items-center gap-3 pl-8">
        <button
          className="bg-[var(--warn)]/15 hover:bg-[var(--warn)]/25 text-[var(--warn)] font-semibold text-sm px-4 py-2 rounded-lg flex items-center gap-2 transition-colors focus:ring-2 ring-[var(--warn)]/30 outline-none"
          onClick={() => onContinuar(activa.id)}
        >
          Retomar Importación Central <ArrowRight className="w-4 h-4" />
        </button>
        <button
          className="bg-[var(--surface)] hover:bg-[var(--warn)]/10 text-[var(--warn)] border border-[var(--warn)]/30 font-semibold text-sm px-4 py-2 rounded-lg transition-colors flex items-center gap-2 focus:ring-2 outline-none"
