...
export function TablaComparacion({ grupos, selecciones, onSelectCandidato, onRemapClick, onDesvincularClick }: Props) {
  ...
  const computeDelta = (nuevo: number, anterior: number) => {
     if (!anterior) return <span className="text-[9px] font-bold text-[var(--ok)] bg-emerald-100/50 px-1 py-0.5 rounded">NUEVO</span>;
  ...
  return (
    <div className="flex flex-col gap-6">
      {grupos.map((g) => {
        ...
        return (
          <div key={g.clave} className="flex bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm text-sm ring-1 ring-slate-100 hover:ring-indigo-100 transition-shadow">
                       ...
                       return (
                         <div key={t} className="flex flex-col justify-center px-3 border-l border-[var(--border)]/60 first:border-l-0 min-w-0">
                            ...
                            <div className="flex flex-wrap items-baseline gap-1.5">
                              <span className={cn("text-sm font-semibold tabular-nums", pn ? "text-[var(--text)]" : "text-slate-300")}>{formatPrice(pn?.valor, pn?.moneda)}</span>
                              {pn && (
                ...
                <div className="flex items-stretch gap-2 px-4 bg-[var(--bg)]/60">
                   ...
                   <div className="flex-1 grid py-2" style={{ gridTemplateColumns: `repeat(${tiersGrupo.length}, minmax(120px, 1fr))` }}>
                      {tiersGrupo.map(t => {
                         ...
                         return (
                           <div key={t} className="flex flex-col justify-center px-3 border-l border-[var(--border)]/60 first:border-l-0 min-w-0">
                              <span className={cn("text-[11px] font-semibold tabular-nums", pa ? "text-[var(--text-muted)]" : "text-slate-300 italic")}>
               ...
               <div className="flex justify-between items-start">
                  <div className="flex flex-col gap-1.5">
                     {seleccionadoId && g.candidatos_jsonb.find(c => c.articulo_id === seleccionadoId)?.metodo_match === 'manual' ? (
                        ...
                        <span className="inline-flex w-fit items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-[var(--ok)]">
                          <CheckCircle2 className="w-3.5 h-3.5" /> EXACTO
                        </span>
                     ) : g.estado_grupo === 'match_similitud' ? (
                        <span className="inline-flex w-fit items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-[var(--warn)]">
                          <AlertCircle className="w-3.5 h-3.5" /> REVISAR
                        </span>
                     ) : (
                        <span className="inline-flex w-fit items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-rose-100 text-[var(--err)]">
                     ...
                     {g.catalogo_sugerido && (
                       <div className="flex items-center gap-2 mt-0.5">
                         {(() => {
                           ...
                           const color = s >= 90 ? 'bg-emerald-100/80 text-[var(--ok)]' : s >= 60 ? 'bg-amber-100/80 text-[var(--warn)]' : 'bg-rose-100/80 text-[var(--err)]';
                           return (
               ...
               <div className="flex flex-col gap-1 w-full relative mt-1">
                 {g.articulo_id_final ? (
                    <div className="flex items-center justify-between bg-[var(--ok)]/10 border border-[var(--ok)]/30 rounded-lg px-2 py-2 w-full">
                       ...
                       <button onClick={() => onDesvincularClick(g.clave)} className="text-[10px] bg-rose-100 hover:bg-rose-200 text-[var(--err)] font-bold px-2 py-1 rounded transition-colors">
                          Desvincular
                       </button>
                    </div>
                 ) : (
                 <select
                   className={cn(
                     ...
                     seleccionadoId ? "border-[var(--accent)]/70 text-indigo-900 ring-2 ring-indigo-50" : "border-slate-300 text-[var(--text-muted)]"
                   )}
