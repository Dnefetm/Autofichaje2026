...
<h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Mapa de procesos - Autofichaje2026</h1>
...
<div style={{ flex: '1 1 160px', border: '1px solid #e2e8f0', borderRadius: 8, padding: 14, background: '#f8fafc' }}><div style={{ fontSize: 12, color: '#64748b' }}>Tablas mapeadas</div><div style={{ fontSize: 26, fontWeight: 700 }}>{data?.resumen?.tablas ?? '-'}</div></div>
<div style={{ flex: '1 1 160px', border: '1px solid #e2e8f0', borderRadius: 8, padding: 14, background: '#f8fafc' }}><div style={{ fontSize: 12, color: '#64748b' }}>Conexiones FK</div><div style={{ fontSize: 26, fontWeight: 700 }}>{data?.resumen?.aristas ?? '-'}</div></div>
<div style={{ flex: '1 1 160px', border: '1px solid ' + (rotas > 0 ? '#dc2626' : '#16a34a'), borderRadius: 8, padding: 14, background: rotas > 0 ? '#fef2f2' : '#f0fdf4' }}><div style={{ fontSize: 12, color: '#64748b' }}>Cadenas rotas</div><div style={{ fontSize: 26, fontWeight: 700, color: rotas > 0 ? '#dc2626' : '#16a34a' }}>{rotas}</div></div>
<div style={{ flex: '1 1 160px', border: '1px solid #f59e0b', borderRadius: 8, padding: 14, background: '#fffbeb' }}><div style={{ fontSize: 12, color: '#64748b' }}>Relaciones sin uso</div><div style={{ fontSize: 26, fontWeight: 700, color: '#b45309' }}>{sinUso}</div></div>
</div>
<h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Como funciona el sistema, paso a paso</h2>
<p style={{ fontSize: 13, color: '#64748b', marginTop: 0 }}>Cada tarjeta describe un macro-proceso del pipeline. Las descripciones se guardan en la tabla mapa_diccionario y son editables sin tocar codigo.</p>
{procesos.length === 0 && <p style={{ fontSize: 13, color: '#94a3b8' }}>Cargando descripciones del diccionario...</p>}
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
{procesos.map((p) => (<div key={p.clave} style={{ border: '1px solid #cbd5e1', borderRadius: 10, padding: 14, background: '#fff' }}>
<div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}><span style={{ background: '#2563eb', color: '#fff', borderRadius: 6, padding: '2px 8px', fontSize: 12, fontWeight: 700 }}>{p.clave}</span><b style={{ fontSize: 15 }}>{p.nombre}</b></div>
<div style={{ fontSize: 13, color: '#334155', marginBottom: 8 }}>{p.descripcion}</div>
{p.proposito && <div style={{ fontSize: 12, color: '#475569' }}><b>Proposito:</b> {p.proposito}</div>}
{p.entradas && <div style={{ fontSize: 12, color: '#475569' }}><b>Entradas:</b> {p.entradas}</div>}
{p.salidas && <div style={{ fontSize: 12, color: '#475569' }}><b>Salidas:</b> {p.salidas}</div>}
{p.responsable && <div style={{ fontSize: 12, color: '#475569' }}><b>Responsable:</b> {p.responsable}</div>}
</div>))}
</div>
{huerfanasNeg.length > 0 && (<div style={{ marginTop: 20, border: '1px solid #f59e0b', background: '#fffbeb', borderRadius: 8, padding: 14 }}><b style={{ color: '#b45309' }}>Atencion:</b> <span style={{ fontSize: 13 }}>Hay {huerfanasNeg.length} tablas con datos que no tienen relacion (FK) declarada. Se conectan solo por convencion, lo que implica riesgo de integridad: {huerfanasNeg.map((h) => h.tabla).join(', ')}.</span></div>)}
</div>)}
{vista === 'catalogo' && (<div>
<div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
<input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder='Buscar tabla, proceso o etapa...' style={{ padding: '8px 12px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13, minWidth: 280 }} />
<span style={{ fontSize: 12, color: '#64748b' }}>{tablasFiltradas.length} de {tablasDicc.length} tablas</span>
</div>
{etapas.map((et) => (<div key={et} style={{ marginBottom: 18 }}>
<h3 style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', borderBottom: '2px solid #e2e8f0', paddingBottom: 4, marginBottom: 8 }}>{et}</h3>
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 10 }}>
{tablasFiltradas.filter((d) => (d.etapa || 'Sin etapa') === et).map((d) => { const cnt = data?.conteos?.[d.clave]; return (<div key={d.clave} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 12, background: '#fff' }}>
<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}><b style={{ fontSize: 14 }}>{d.nombre}</b>{cnt != null && <span style={{ fontSize: 11, color: cnt === 0 ? '#dc2626' : '#16a34a', whiteSpace: 'nowrap' }}>{fmt(cnt)} filas</span>}</div>
<code style={{ fontSize: 11, color: '#7c3aed' }}>{d.clave}</code>
<div style={{ fontSize: 12, color: '#334155', marginTop: 6 }}>{d.descripcion}</div>
{d.proposito && <div style={{ fontSize: 11, color: '#64748b', marginTop: 4 }}><b>Proposito:</b> {d.proposito}</div>}
<div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>{d.responsable ? 'Responsable: ' + d.responsable : ''}</div>
</div>); })}
</div>
</div>))}
...
<div style={{ display: 'flex', gap: 16, fontSize: 12, marginBottom: 12 }}><span style={{ color: '#16a34a' }}>Conexion viva</span><span style={{ color: '#dc2626' }}>Conexion rota (tabla destino vacia)</span><span style={{ color: '#9ca3af' }}>Sin uso (origen vacio)</span></div>
...
{aristas.map((a, i) => { const o = POS[a.origen]; const d = POS[a.destino]; if (!o || !d) return null; const x1 = o.x + NW / 2, y1 = o.y + NH / 2, x2 = d.x + NW / 2, y2 = d.y + NH / 2; const c = COLOR[a.estado] || '#94a3b8'; return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={c} strokeWidth={a.estado === 'viva' ? 1.5 : 2} strokeDasharray={a.estado === 'viva' ? '0' : '5,4'} opacity={0.75} style={{ cursor: 'pointer' }} onClick={() => setSel(a)} />; })}
{nodos.map((n) => { const p = POS[n]; const cnt = data?.conteos?.[n]; const vacio = cnt === 0; return (<g key={n} onClick={() => setSel({ origen: n, col: '', destino: '', estado: vacio ? 'sin_uso' : 'viva', filas_origen: cnt ?? null, filas_destino: null })} style={{ cursor: 'pointer' }}><rect x={p.x} y={p.y} width={NW} height={NH} rx={8} fill={vacio ? '#fef2f2' : '#fff'} stroke={vacio ? '#dc2626' : '#334155'} strokeWidth={1.5} /><text x={p.x + 8} y={p.y + 18} fontSize={11} fontWeight={700} fill='#0f172a'>{p.label}</text><text x={p.x + 8} y={p.y + 34} fontSize={10} fill={vacio ? '#dc2626' : '#16a34a'}>{n}: {fmt(cnt ?? null)}</text></g>); })}
</svg>
</div>
{sel && (<div style={{ marginTop: 12, padding: 12, border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', fontSize: 13 }}>{sel.col ? (<div><b>Conexion:</b> {sel.origen}.{sel.col} -> {sel.destino} &nbsp; <span style={{ color: COLOR[sel.estado] }}>[{sel.estado.toUpperCase()}]</span><br/>Filas origen ({sel.origen}): <b>{fmt(sel.filas_origen)}</b> | Filas destino ({sel.destino}): <b>{fmt(sel.filas_destino)}</b>{sel.estado === 'rota' && <div style={{ color: '#dc2626', marginTop: 4 }}>La tabla destino esta vacia: esta relacion no puede resolverse. Cadena rota aqui.</div>}{descPorClave[sel.origen]?.descripcion && <div style={{ marginTop: 6, color: '#475569' }}><b>{sel.origen}:</b> {descPorClave[sel.origen]?.descripcion}</div>}</div>) : (<div><b>Tabla:</b> {sel.origen} - Filas: <b>{fmt(sel.filas_origen)}</b>{descPorClave[sel.origen]?.descripcion && <div style={{ marginTop: 6, color: '#475569' }}>{descPorClave[sel.origen]?.descripcion}</div>}</div>)}</div>)}
...
<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>{(data?.huerfanas ?? []).map((h) => (<div key={h.tabla} style={{ border: '1px dashed #f59e0b', background: '#fffbeb', borderRadius: 6, padding: '6px 10px', fontSize: 12 }}><b>{h.tabla}</b> <span style={{ color: '#92400e' }}>({fmt(h.filas)} filas)</span></div>))}</div>
</div>)}
</div>);
}
