...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 12 }}>{procesos.map((p) => (<div key={p.clave} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 14 }}><div style={{ fontSize: 12, color: '#2563eb', fontWeight: 700 }}>{p.clave}</div><div style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>{p.nombre}</div><div style={{ fontSize: 13, color: '#334155' }}>{p.descripcion}</div>{p.proposito && <div style={{ fontSize: 12, color: '#64748b', marginTop: 6 }}><b>Proposito:</b> {p.proposito}</div>}{p.entradas && <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}><b>Entradas:</b> {p.entradas}</div>}{p.salidas && <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}><b>Salidas:</b> {p.salidas}</div>}{p.responsable && <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}><b>Responsable:</b> {p.responsable}</div>}</div>))}</div>
{huerfanasNeg.length > 0 && (<div style={{ marginTop: 20, border: '1px solid #f59e0b', background: '#fffbeb', borderRadius: 8, padding: 14 }}><b>Atencion:</b> Hay {huerfanasNeg.length} tablas con datos que no tienen relacion (FK) declarada. Se conectan solo por convencion, lo que implica riesgo de integridad: {huerfanasNeg.map((h) => h.tabla).join(', ')}.</div>)}
</div>)}
{vista === 'catalogo' && (<div>
...
{etapas.map((et) => (<div key={et} style={{ marginBottom: 18 }}><h3 style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', borderBottom: '2px solid #e2e8f0', paddingBottom: 4, marginBottom: 8 }}>{et}</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 10 }}>{tablasFiltradas.filter((d) => (d.etapa || 'Sin etapa') === et).map((d) => { const cnt = data?.conteos?.[d.clave]; return (<div key={d.clave} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}><b style={{ fontSize: 14 }}>{d.nombre}</b>{cnt != null && <span style={{ fontSize: 11, color: '#64748b' }}>{fmt(cnt)} filas</span>}</div><code style={{ fontSize: 11, color: '#7c3aed' }}>{d.clave}</code><div style={{ fontSize: 12, color: '#334155', marginTop: 6 }}>{d.descripcion}</div>{d.proposito && <div style={{ fontSize: 11, color: '#64748b', marginTop: 4 }}><b>Proposito:</b> {d.proposito}</div>}<div style={{ fontSize: 11, color: '#94a3b8', marginTop: 4 }}>{d.responsable ? 'Responsable: ' + d.responsable : ''}</div></div>); })}</div></div>))}
{tablasFiltradas.length === 0 && <p style={{ fontSize: 13, color: '#94a3b8' }}>Sin resultados para tu busqueda.</p>}
</div>)}
{vista === 'tecnico' && (<div>
{data?.resumen && <div style={{ fontSize: 13, marginBottom: 10 }}>Tablas: <b>{data.resumen.tablas}</b> | Conexiones FK: <b>{data.resumen.aristas}</b> | <span style={{ color: '#dc2626' }}>Rotas (destino vacio): <b>{data.resumen.rotas}</b></span> | <span style={{ color: '#94a3b8' }}>Sin uso (origen vacio): <b>{data.resumen.sin_uso}</b></span></div>}
<div style={{ display: 'flex', gap: 16, fontSize: 12, marginBottom: 12 }}><span style={{ color: '#16a34a' }}>Conexion viva</span><span style={{ color: '#dc2626' }}>Conexion rota (tabla destino vacia)</span><span style={{ color: '#94a3b8' }}>Sin uso (origen vacio)</span></div>
<div style={{ overflow: 'auto', border: '1px solid #e2e8f0', borderRadius: 8, background: '#fafafa' }}><svg width={1480} height={780} style={{ display: 'block' }}>{aristas.map((a, i) => { const o = POS[a.origen]; const d = POS[a.destino]; if (!o || !d) return null; const x1 = o.x + NW / 2, y1 = o.y + NH / 2, x2 = d.x + NW / 2, y2 = d.y + NH / 2; const c = COLOR[a.estado] || '#94a3b8'; return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={c} strokeWidth={1.5} strokeDasharray={a.estado === 'sin_uso' ? '4 4' : undefined} style={{ cursor: 'pointer' }} onClick={() => setSel(a)} />; })}{nodos.map((n) => { const p = POS[n]; const cnt = data?.conteos?.[n]; const vacio = cnt === 0; return (<g key={n} onClick={() => setSel({ origen: n, col: '', destino: '', estado: vacio ? 'sin_uso' : 'viva', filas_origen: cnt ?? null, filas_destino: null })} style={{ cursor: 'pointer' }}><rect x={p.x} y={p.y} width={NW} height={NH} rx={6} fill='#fff' stroke={vacio ? '#dc2626' : '#0f172a'} strokeWidth={1.5} /><text x={p.x + 8} y={p.y + 18} fontSize={11} fontWeight={700} fill='#0f172a'>{p.label}</text><text x={p.x + 8} y={p.y + 34} fontSize={10} fill='#64748b'>{n}: {fmt(cnt ?? null)}</text></g>); })}</svg></div>
{sel && (<div style={{ marginTop: 12, padding: 12, border: '1px solid #cbd5e1', borderRadius: 8, background: '#fff', fontSize: 13 }}>{sel.col ? (<div><div><b>Conexion:</b> {sel.origen}.{sel.col} {'->'} {sel.destino}   [{sel.estado.toUpperCase()}]</div><div style={{ marginTop: 4 }}>Filas origen ({sel.origen}): <b>{fmt(sel.filas_origen)}</b> | Filas destino ({sel.destino}): <b>{fmt(sel.filas_destino)}</b></div>{sel.estado === 'rota' && <div style={{ color: '#dc2626', marginTop: 4 }}>La tabla destino esta vacia: esta relacion no puede resolverse. Cadena rota aqui.</div>}{descPorClave[sel.origen]?.descripcion && <div style={{ marginTop: 6, color: '#475569' }}><b>{sel.origen}:</b> {descPorClave[sel.origen]?.descripcion}</div>}</div>) : (<div><div><b>Tabla:</b> {sel.origen} - Filas: <b>{fmt(sel.filas_origen)}</b></div>{descPorClave[sel.origen]?.descripcion && <div style={{ marginTop: 6, color: '#475569' }}>{descPorClave[sel.origen]?.descripcion}</div>}</div>)}</div>)}
...
<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>{(data?.huerfanas ?? []).map((h) => (<div key={h.tabla} style={{ border: '1px solid #e2e8f0', borderRadius: 6, padding: '6px 10px', fontSize: 12 }}><b>{h.tabla}</b> ({fmt(h.filas)} filas)</div>))}</div>
</div>)}
</div>);
}
