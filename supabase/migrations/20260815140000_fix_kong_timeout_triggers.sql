-- ---------------------------------------------------------------------
-- Fix: Kong API Gateway Timeout on massive bulk UPSERTs.
-- En lugar de disparar miles de row-level triggers por cada precio insertado,
-- deshabilitamos los triggers temporalmente y encolamos los recalculos de forma masiva (Bulk Insert).
-- Fix 2: Corregido error semantico al comparar codigo_excel con modelo_norm.
-- Fix 3: Removido ON CONFLICT invalido en precio_recalc_queue.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  -- --------------------------------------------------------
  -- FASE RESUELTOS (Optimizada separando OR en dos uniones + Bulk Enqueue)
  -- --------------------------------------------------------
  
  -- DESACTIVAR triggers temporalmente para evitar el cuello de botella del Timeout de Kong (API)
  ...
  upsert_costos AS (
    INSERT INTO costos_articulo (
      ...
      valor, moneda, fuente, estado_match, vigente, incluye_iva, actualizado_el
    )
    SELECT
      articulo_id, importacion_id, modelo_excel, marca_excel,
      codigo_universal_excel, descripcion_excel, tipo_costo,
      valor, moneda, fuente, estado_match, vigente, incluye_iva, now()
    FROM dedup
  ...
  enqueue AS (
    INSERT INTO precio_recalc_queue (articulo_id, prioridad)
    SELECT DISTINCT articulo_id, 10
    FROM upsert_costos
  )
  ...
  -- --------------------------------------------------------
  -- FASE HUERFANOS (PENDIENTES) - Optimizada sin OR
  -- --------------------------------------------------------
  WITH huerfanos AS (
