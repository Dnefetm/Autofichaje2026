...
-- 1. DROP both overloads to ensure a clean slate
DROP FUNCTION IF EXISTS public.fn_match_precios_v2(uuid, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS public.fn_match_precios_v2(uuid) CASCADE;

-- 2. Drop the orphaned table just in case it was recreated by the rogue subagent
DROP TABLE IF EXISTS public.lista_precios_proveedor CASCADE;

-- 3. Run the correct definition of fn_match_precios_v2
...
BEGIN
    ...
    VALUES (p_importacion_id, 'MATCHING_COMPLETO', 'Matching Set-Based con Ola 0 y Fase 0 concluido.');
...
-- 4. VALIDATION QUERIES FOR COMET
SELECT json_agg(t) FROM (
  SELECT oid, proname, pg_get_function_identity_arguments(oid) as args, length(pg_get_functiondef(oid)) as len
  FROM pg_proc WHERE proname='fn_match_precios_v2'
) t;

SELECT json_agg(t) FROM (
  SELECT proname, length(pg_get_functiondef(oid)) as len FROM pg_proc WHERE pg_get_functiondef(oid) ~ '\mlista_precios_proveedor\M'
) t;
