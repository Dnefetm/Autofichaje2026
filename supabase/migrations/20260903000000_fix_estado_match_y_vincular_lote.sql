-- =============================================================================
-- MIGRACIÓN (append-only): sana estado_match='completado' y el cast ::UUID
-- =============================================================================
-- NO edita migraciones anteriores. Corre al final y pisa (CREATE OR REPLACE) las
-- definiciones buggy de las 3 funciones VIVAS. No toca datos (solo definiciones).
--
-- Funciones corregidas:
--   1. fn_vincular_lote            -> articulo_id TEXT (quita ::UUID)
--   2. fn_tg_promote_pendientes    -> estado_match 'match_exacto' (era 'completado')
--   3. fn_drain_costos_pendientes_sin_match -> 'match_exacto' (era 'completado')
--
-- VALIDADO (función #3): el body se obtuvo de producción vía pg_get_functiondef y
-- coincide con el real (versión iterativa, ON CONFLICT (articulo_id, tipo_costo,
-- fuente)). Se corrigió el body de la migración para usar esa fuente real.
--
-- Idempotente y reversible (ver 20260903000001_rollback_...).
-- =============================================================================
BEGIN;

-- 1. fn_vincular_lote: articulo_id TEXT, sin cast ::UUID ---------------------
...
-- 2. fn_tg_promote_pendientes: estado_match 'match_exacto' --------------------
...
-- 3. fn_drain_costos_pendientes_sin_match: estado_match 'match_exacto' --------
-- Cuerpo REAL de producción (obtenido vía pg_get_functiondef). Único cambio:
-- 'completado' -> 'match_exacto'. Conserva ON CONFLICT (articulo_id, tipo_costo,
-- fuente) y la propagación por GTIN.
    ...
    LOOP
        ...
        LOOP
            ...
            VALUES (
                ...
                v_row.tipo_costo, v_row.valor, v_row.moneda, 'excel', 'match_exacto', true
            )
