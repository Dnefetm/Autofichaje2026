-- =============================================================================
-- MIGRACIÓN (append-only) Capa 0 — parte 9: re-materialización incremental
-- =============================================================================
-- fn_materializar_vinculacion reconstruye TODO el lote (~15k filas, ~3.7s).
-- fn_materializar_vinculacion_filas reconstruye SOLO las filas indicadas,
-- para que vincular/desvincular/rechazar cueste O(filas afectadas), no O(lote).
-- Misma lógica de clasificación que 0016 (mapeo dinámico + rechazo + a_triple).
-- =============================================================================
BEGIN;

CREATE OR REPLACE FUNCTION public.fn_materializar_vinculacion_filas(
...
BEGIN
    IF p_fila_nums IS NULL OR cardinality(p_fila_nums) = 0 THEN
        RETURN;
    END IF;
    ...
    -- Borra solo las filas afectadas y las re-clasifica.
    DELETE FROM vinculacion_clasificada
    WHERE importacion_id = p_importacion_id AND fila_num = ANY(p_fila_nums);
    ...
    WITH raw AS (
        ...
        WHERE r.importacion_id = p_importacion_id AND r.fila_num = ANY(p_fila_nums)
    ),
    alias_lock AS (
...
-- ROLLBACK:
-- DROP FUNCTION IF EXISTS public.fn_materializar_vinculacion_filas(uuid, text, int[]);
