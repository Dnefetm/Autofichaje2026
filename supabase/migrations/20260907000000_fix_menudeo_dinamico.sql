-- =============================================================================
-- Fase 0.3 — Menudeo dinámico en la materialización de vinculación
-- =============================================================================
-- fn_materializar_vinculacion leía el menudeo SIEMPRE de la columna fija
-- 'PRECIO MENUDEO (CON IVA)'. Eso dejaba en 0 el precio de los artículos cuyo
-- proveedor usa otra columna para el menudeo (la que el operador definió en
-- mapeo_columnas.precios[] con tipo_costo='menudeo').
-- Ahora la columna de menudeo se lee desde el mapeo dinámico.
-- =============================================================================
...
DECLARE
    ...
    v_col_menudeo     text := 'PRECIO MENUDEO (CON IVA)';
BEGIN
    ...
    -- Menudeo dinámico: columna mapeada como tipo_costo='menudeo'
    SELECT COALESCE(NULLIF(precio->>'columna', ''), v_col_menudeo)
    INTO v_col_menudeo
    ...
    WITH raw AS (
        SELECT
            ...
            COALESCE(NULLIF(regexp_replace(r.payload->>v_col_menudeo, '[^0-9.]', '', 'g'), ''), '0')::numeric AS menudeo
...
-- Re-materializar el lote más reciente de cada proveedor para corregir menudeo.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT DISTINCT ON (proveedor) id, proveedor
    FROM importaciones_excel
    ORDER BY proveedor, creado_el DESC
  LOOP
    PERFORM public.fn_materializar_vinculacion(r.id, r.proveedor);
  END LOOP;
END $$;
