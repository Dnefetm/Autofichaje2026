-- =============================================================================
-- MIGRACIÓN (append-only): Motor V2 (post-publicación) lee costo de la capa canónica
-- =============================================================================
-- Problema: fn_recalcular_precio_publicacion (botón "Recalcular" de la
--           Auditoría de Precio) sacaba el costo de `costos_articulo` (tabla
--           legacy que el flujo nuevo ya NO puebla). Para artículos vinculados
--           solo bajo el flujo nuevo, costos_articulo no tenía 'menudeo' y el
--           draft quedaba en 'missing_cost' → "Falta Costo Base" → precio 0.
--
-- Fix: reemplazar el CTE `costo_unico` (sobre costos_articulo) por la capa
--      canónica fn_costo_articulo_vigente (precios_proveedor ⨝ alias), por
--      artículo, multiplicado por cantidad y sumado. Idéntico resto de lógica.
--
-- Idempotente (CREATE OR REPLACE). No altera tablas ni otras funciones.
-- Dependencia: fn_costo_articulo_vigente (migración 20260907000001).
-- =============================================================================
...
BEGIN
  ...
  -- Reemplaza al CTE `costo_unico` que leía costos_articulo (legacy).
