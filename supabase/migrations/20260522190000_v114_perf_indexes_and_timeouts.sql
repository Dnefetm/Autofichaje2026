-- Migration: v114_perf_indexes_and_timeouts
-- Fecha: 2026-05-22
-- Propósito: 
-- 1. Crear función f_unaccent_immutable para poder crear índices funcionales.
-- 2. Crear índices funcionales y estándar para evitar seq scans en matching.
-- 3. Actualizar funciones de resolución para usar f_unaccent_immutable.
-- 4. Aumentar statement_timeout localmente en las funciones a 180s.

BEGIN;

-- =====================================================================
-- 1. Función inmutable wrapper para unaccent
-- =====================================================================
...
-- =====================================================================
-- 2. Creación de índices clave
-- =====================================================================

-- proveedor_articulos_alias
...
-- listas_precios_raw y staging
CREATE INDEX IF NOT EXISTS ix_lpr_imp  ON listas_precios_raw (importacion_id);
CREATE INDEX IF NOT EXISTS ix_lprs_imp ON listas_precios_raw_staging (importacion_id);

-- Índices funcionales usando la función inmutable
...
-- =====================================================================
-- 3. Actualizar fn_resolver_y_poblar_costos (Timeout + f_unaccent_immutable)
-- =====================================================================
           ...
           (
             SELECT a.articulo_id
              ...
              WHERE a.proveedor   = p_proveedor
                ...
                AND ( lower(public.f_unaccent_immutable(trim(a.codigo_excel))) = lower(public.f_unaccent_immutable(trim(f.payload->>v_col_modelo)))
                   OR lower(public.f_unaccent_immutable(trim(a.modelo_excel))) = lower(public.f_unaccent_immutable(trim(f.payload->>v_col_modelo))) )
              LIMIT 1
           ) AS articulo_id
      FROM filas f
  ),
     ...
     WHERE NOT EXISTS (
       ...
       AND NOT EXISTS (
             SELECT 1 FROM articulos ar
              WHERE lower(public.f_unaccent_immutable(trim(ar.modelo))) = lower(public.f_unaccent_immutable(trim(f.payload->>v_col_modelo)))
           )
  ),
  expand AS (
    SELECT h.payload,
...
-- =====================================================================
-- 4. Actualizar fn_match_precios_v2 (Timeout + f_unaccent_immutable)
-- =====================================================================
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(
            ...
            (SELECT a3.articulo_id FROM articulos a3 WHERE lower(public.f_unaccent_immutable(trim(a3.codigo_universal))) = lower(public.f_unaccent_immutable(trim(e.codigo_excel))) AND e.codigo_excel <> '' AND a3.activo = true LIMIT 1)
